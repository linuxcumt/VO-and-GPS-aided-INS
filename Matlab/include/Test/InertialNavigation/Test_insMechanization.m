...
% Rotate about the body-frame x axis for T seconds to get roll the device
% to zero roll
...
% Initialize state
x0 = zeros(25,1);
x0([1,4,7],1) = [1; 0; 0]; % Arbitrary position vector
q = [1; 0; 0; 0]; % Set heading, pitch, roll to 0
q = q/norm(q); % Normalize quaternion
x0([10,11,12,13],1) = q; % Attitude
...
N = T/dt;
zhist = zeros(6,N);
zhist(4,:) = z; % Apply rotation to x axis gyro
...
%% Test 4
% Rotate about y axis to achieve pitch angle of 45 degrees after last
% manuever
...
% Check attitude
assert(abs(attitude(1) - d2) < 1e-1, 'Bad heading');
