...
% Initialize state
x0 = zeros(22,1);
x0(1:3,1) = [1; 2; 3]; % Arbitrary position vector
...
% Initialize state
x0 = zeros(22,1);
x0(1:3,1) = [100; 0; 0]; % Arbitrary position vector
x0(4:6,1) = [2; 0; 0]; % Arbitrary velocity vector
...
assert(abs(xhist(4,end) - vf) < 1e-1, 'Bad final velocity');
...
% Initialize state
x0 = zeros(22,1);
x0(1:3,1) = 6400e3*[1; 0; 0]; % Arbitrary position vector
q = [1; 0; 0; 0]; % Set heading, pitch, roll to 0
q = q/norm(q); % Normalize quaternion
x0(7:10,1) = q; % Attitude
